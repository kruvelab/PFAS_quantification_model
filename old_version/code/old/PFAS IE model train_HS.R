...
data = Orbitrap_dataset_raw %>%
  ...
  mutate(RT = as.numeric(RT),
         area_IC = Area*IC,
         Theoretical_conc_uM = Theoretical_amt/MW) %>%       
...
IE_slope_cor = ggplot() +
  geom_point(data = logIE_pred_model$data$training_set,
             mapping = aes(logIE, logIE_predicted),
             color = "light grey",
             alpha = 0.5,
             size = 3) +
  geom_point(data = logIE_pred_model$data$test_set,
             mapping = aes(logIE, logIE_predicted),
             color = "blue",
             alpha = 0.5,
             size = 3) +
...
## Quantification based on most similar compound

SMILES_to_molecularFormula

#Insert smiles to function - counts C and F, finds the closest, prints the name of the closest, quantfies using this 
# compounds IE value? For comparison?
